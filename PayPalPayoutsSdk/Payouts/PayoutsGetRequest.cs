...
namespace PayoutsSdk.Payouts
{
    ...
    /// </summary>
    public class PayoutsGetRequest : HttpRequest
    {
        public PayoutsGetRequest(string PayoutBatchId) : base("/v1/payments/payouts/{payout_batch_id}?", HttpMethod.Get, typeof(PayoutBatch))
        {
            try {
                this.Path = this.Path.Replace("{payout_batch_id}", Uri.EscapeDataString(Convert.ToString(PayoutBatchId) ));
            } catch (IOException) {}
        ...
        public PayoutsGetRequest Page(int Page)
        {
        ...
        public PayoutsGetRequest PageSize(int PageSize)
        {
        ...
        public PayoutsGetRequest TotalRequired(bool TotalRequired)
        {
